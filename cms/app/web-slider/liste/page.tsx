...
export default function WebSliderListePage() {
    ...
    const handleDelete = async () => {
        if (!selectedId) {
            toast({
                variant: "destructive",
                title: "Hata",
                description: "Silinecek slider ID'si bulunamadı."
            });
            return;
        }
        ...
        try {
            ...
            const deleteUrl = `${API_ENDPOINTS.webSliderDelete}${selectedId}`;
        ...
        },
        {
            ...
            cell: ({ row }) => (
                <div className="w-20 h-20">
                    <Image src={`${API_BASE_URL_SLIDER_RESIM}/${row.original.images}`} alt={row.original.title} width={50} height={50} />
                </div>
            )
        },
        {
    ...
    const fetchData = async () => {
        try {
            setIsLoading(true);
            const response = await api.get(API_ENDPOINTS.webSliderListe);
    ...
    return (
        <div>
            ...
            <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
                <DialogContent>
                    <DialogHeader>
                        <DialogTitle>Slider Sil</DialogTitle>
                    </DialogHeader>
                    <p>Bu slider'ı silmek istediğinize emin misiniz?</p>
                    <DialogFooter>
            ...
            <div className="flex justify-between items-center mb-3">
                ...
                <div>
                    <Link href="/web-slider">
                        <Button>
                            <PlusIcon className="w-4 h-4" /> Yeni Slider
                        </Button>
                    </Link>
                </div>
            </div>
                        ...
                        {isLoading ? (
                            ...
                            table.getRowModel().rows.map((row) => (
                                <TableRow
                                    ...
                                    onClick={() => router.push(`/web-slider/${row.original.id}`)}
