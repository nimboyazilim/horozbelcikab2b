...
export default function PopupDuzenlePage() {
    ...
    const [originalActive, setOriginalActive] = useState<string | null>(null);
    ...
    const fetchPopup = async () => {
        try {
            const response = await api.get(`/popup/${params.slug}`);
            ...
            setFormData({
                ...
                order1: data.order1.toString(),
                image: null
            });
            setCurrentImage(data.image);
            setOriginalActive(data.is_active.toString());
        } catch (error) {
            toast({
                variant: "destructive",
                title: "Hata",
                description: "Pop-up yüklenirken bir hata oluştu."
            });
        }
    };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const activeDegistu = originalActive !== null && originalActive !== formData.is_active;
            let logMesaj: string;
            if (activeDegistu) {
                logMesaj = formData.is_active === '1'
                    ? `Popup aktife çekildi: "${formData.title_tr}"`
                    : `Popup pasife çekildi: "${formData.title_tr}"`;
            } else {
                logMesaj = `Popup güncellendi: "${formData.title_tr}"`;
            }

            await api.put(`/popup/${params.slug}`, submitData, {
                headers: { 'Content-Type': 'multipart/form-data' },
                _logAction: { action: logMesaj, category: 'Popup' },
            } as object);

            toast({
                variant: "default",
                title: "Başarılı",
                description: "Pop-up başarıyla güncellendi."
            });
            ...
            toast({
                variant: "destructive",
                title: "Hata",
                description: "Pop-up güncellenirken bir hata oluştu."
            });
        } finally {
            setIsLoading(false);
        }
    ...
    const breadcrumbData = [
        ...
        { name: 'Pop-up Düzenle', link: `/popup/${params.slug}` },
    ];
    ...
    return (
        <div>
            ...
            <div className="flex justify-between items-center mb-4 mt-8">
                <h1 className="text-3xl font-bold tracking-tight">Pop-up Düzenle</h1>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                <Card>
                    ...
                    <CardContent className="space-y-4">
                        {currentImage && !previewImage && (
                            <div className="mb-4">
                                ...
                                <div className="relative w-64 h-64 mt-2">
                                    <Image
                                        src={`${API_BASE_URL_POPUP}/${currentImage}`}
                                        alt="Current"
                                        fill
                        ...
                        <div>
                            <Label htmlFor="image">Yeni Görsel Seç (Opsiyonel)</Label>
                            <Input
                                id="image"
                                type="file"
                                accept="image/*"
